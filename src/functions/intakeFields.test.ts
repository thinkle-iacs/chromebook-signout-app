...
describe("field mapping", () => {
  ...
  test("formatMac writes colon-separated upper case, and leaves odd input alone", () => {
    expect(formatMac("a0b1c2d3e4f5")).toBe("A0:B1:C2:D3:E4:F5");
    expect(formatMac("A0-B1-C2-D3-E4-F5")).toBe("A0:B1:C2:D3:E4:F5");
  ...
  test("suggestedFields drops what Google didn't report", () => {
    expect(suggestedFields(google)).toEqual({
      ...
      Model: "HP Chromebook 11 G8 EE",
      Make: "HP",
      "MAC-Wireless": "A0:B1:C2:D3:E4:F5",
...
describe("planCommit (design §6)", () => {
  const base: CommitInput = {
    ...
    existing: null,
    tagHolder: null,
    defaults: { Purpose: "Student Loan", Status: "Active", Location: "", Category: "Chromebook" },
    overrides: {},
    allowRetag: false,
  };
  ...
  test("same tag on the same serial is an idempotent refresh that keeps year and defaults", () => {
    ...
    expect(plan.fields["MAC-Wireless"]).toBe("A0:B1:C2:D3:E4:F5");
  });
