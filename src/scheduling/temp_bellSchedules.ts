...
// You can add more schedules for different grade levels
  ...
  schedules: [
    {
      days: [1, 2, 3, 4, 5], // Monday through Friday - same schedule every day for now
      periods: [
        // TODO: Add high school periods when available
        // For now, using same as middle school
        ...MIDDLE_SCHOOL_SCHEDULE.schedules[0].periods,
      ],
    },
  ],
};
...
export function parseProjectScheduleFromSIS(
  ...
  for (const cls of sisSchedule.classes) {
    ...
    for (const periodString of cls.periods) {
      // Parse strings like "BLOCK A(Mon) BLOCK D(Thu)"
      const blockMatches =
